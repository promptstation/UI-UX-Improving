---
name: quality-gates-tests-lint-and-security-in-pipeline
description: Design required-check policies. Use when the user asks to author workflows, enforce conventional commits, automate versioning, sync multi-file versions, or gate releases. Covers Section 5: Quality Gates — Tests, Lint and Security in Pipeline (CI/CD & Release Automation).
...
# Quality Gates — Tests, Lint and Security in Pipeline

## Mission

Deliver the Section 5 capability of CI/CD & Release Automation: design required-check policies. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Design required-check policies.
- Gate releases on tests and security.
- Attest artifact provenance.

Context of this section: Required status checks, test matrices, static analysis, dependency audit, and provenance attestation guarding every release.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 5: Quality Gates — Tests, Lint and Security in Pipeline via Zero-touch release." Do not blend approaches accidentally.
