---
name: dataset-pipelines-rows-to-searchable-documents
description: Build row-to-document ingest pipelines. Use when the user asks to build tokenizers and indexes, implement BM25, tune relevance, or ship zero-dependency search. Covers Section 3: Dataset Pipelines — Rows to Searchable Documents (Local Search Engines).
...
# Dataset Pipelines — Rows to Searchable Documents

## Mission

Deliver the Section 3 capability of Local Search Engines: build row-to-document ingest pipelines. Work from evidence, protect what matters, and prove the result at full size before delivery.
...
Context of this section: Ingesting CSV and JSON rows into searchable documents: field selection, per-field weighting, incremental updates, and persistence without external services.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 3: Dataset Pipelines — Rows to Searchable Documents via Stdlib purity." Do not blend approaches accidentally.
