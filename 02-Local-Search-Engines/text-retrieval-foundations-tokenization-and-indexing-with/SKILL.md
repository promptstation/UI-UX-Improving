---
name: text-retrieval-foundations-tokenization-and-indexing-with
description: Build tokenizers and inverted indexes with stdlib only. Use when the user asks to build tokenizers and indexes, implement BM25, tune relevance, or ship zero-dependency search. Covers Section 1: Text Retrieval Foundations — Tokenization and Indexing with Stdlib (Local Search Engines).
compatibility: Python standard library only, judged query sets, and latency benchmarks; assumes calibrated review conditions and non-destructive, reversible technique.
metadata:
...
# Text Retrieval Foundations — Tokenization and Indexing with Stdlib

## Mission

Deliver the Section 1 capability of Local Search Engines: build tokenizers and inverted indexes with stdlib only. Work from evidence, protect what matters, and prove the result at full size before delivery.
...
Context of this section: Python-standard-library text pipelines: normalization, tokenization, stop handling, and inverted-index construction over dataset rows with zero dependencies.
...
Standard toolkit: Python standard library only, judged query sets, and latency benchmarks.

Available approaches for this section:

- **Stdlib purity** -- inverted indexes with no external APIs
- **BM25 ranking** -- tuned saturation and length norms
- **Measured relevance** -- judged sets driving every change
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 1: Text Retrieval Foundations — Tokenization and Indexing with Stdlib via Stdlib purity." Do not blend approaches accidentally.
...
Work with Python standard library only, judged query sets, and latency benchmarks. Keep every consequential move on its own named, masked, reversible layer or stage. Record settings as logic with reasons, not as bare numbers, so the next piece in the set can adapt them honestly.

Typical working contexts for this skill:

1. ten-thousand-row CSV searchable in milliseconds.
2. typo-tolerant query recovery saving empty results.
3. embedded module integrated by another team.

## Examples

**Example 1:** Input: ten-thousand-row CSV searchable in milliseconds. Actions: declare the approach, rebuild the foundation, execute low-intensity precision passes, protect critical qualities, proof twice. Result: section-standard outcome with documented rationale.

**Example 2:** Input: typo-tolerant query recovery saving empty results. Actions: diagnose against capability targets, repair the foundation, re-apply intent, verify coherence. Result: rescued work meeting this section's bar.

**Example 3:** Input: embedded module integrated by another team. Actions: define the signature on the hero, propagate with adaptation, verify the set. Result: coherent series holding this section's standard throughout.
...
Avoid dependencies smuggled into local search, untuned default scoring presented as relevance, zero-result dead ends without recovery, unmeasured changes to analyzers, index formats changed without versioning.

Reference list:

- dependencies smuggled into local search.
- untuned default scoring presented as relevance.
- zero-result dead ends without recovery.
- unmeasured changes to analyzers.
- index formats changed without versioning.
...
6. Is relevance measured on judged queries?
7. Does it run with stdlib only?
8. Do empty results recover gracefully?
...
Relevance is measured, speed is budgeted. No APIs, no excuses, just ranked rows.
