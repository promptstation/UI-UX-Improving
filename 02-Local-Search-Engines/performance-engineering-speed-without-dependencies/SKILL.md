---
name: performance-engineering-speed-without-dependencies
description: Profile and optimize retrieval hot paths. Use when the user asks to build tokenizers and indexes, implement BM25, tune relevance, or ship zero-dependency search. Covers Section 5: Performance Engineering — Speed Without Dependencies (Local Search Engines).
...
# Performance Engineering — Speed Without Dependencies

## Mission

Deliver the Section 5 capability of Local Search Engines: profile and optimize retrieval hot paths. Work from evidence, protect what matters, and prove the result at full size before delivery.
...
Context of this section: Profiling with cProfile, posting-list optimization, caching hot paths, and memory discipline so local search stays instant on modest hardware.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 5: Performance Engineering — Speed Without Dependencies via Stdlib purity." Do not blend approaches accidentally.
