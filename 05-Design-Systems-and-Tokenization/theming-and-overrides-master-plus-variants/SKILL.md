---
name: theming-and-overrides-master-plus-variants
description: Theme via master-plus-overrides. Use when the user asks to architect tokens, bind palettes to categories, pair typefaces, specify components, or govern systems. Covers Section 6: Theming and Overrides — Master Plus Variants (Design Systems & Tokenization).
...
# Theming and Overrides — Master Plus Variants

## Mission

Deliver the Section 6 capability of Design Systems & Tokenization: theme via master-plus-overrides. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Theme via master-plus-overrides.
- Scope exceptions safely.
- Avoid system forks.

Context of this section: Master themes with scoped overrides: brand variants, seasonal skins, and per-surface exceptions without forking the system.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 6: Theming and Overrides — Master Plus Variants via Tiered tokens." Do not blend approaches accidentally.
