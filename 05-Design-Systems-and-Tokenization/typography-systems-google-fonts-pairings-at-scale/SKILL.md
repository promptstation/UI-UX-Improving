---
name: typography-systems-google-fonts-pairings-at-scale
description: Pair faces by rule not taste. Use when the user asks to architect tokens, bind palettes to categories, pair typefaces, specify components, or govern systems. Covers Section 3: Typography Systems — Google Fonts Pairings at Scale (Design Systems & Tokenization).
...
# Typography Systems — Google Fonts Pairings at Scale

## Mission

Deliver the Section 3 capability of Design Systems & Tokenization: pair faces by rule not taste. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Pair faces by rule not taste.
- Implement performant font loading.
- Protect layouts with fallbacks.

Context of this section: Pairing rules for display and text faces, modular scales, Google Fonts API loading strategy, and fallbacks preserving layout.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 3: Typography Systems — Google Fonts Pairings at Scale via Tiered tokens." Do not blend approaches accidentally.
