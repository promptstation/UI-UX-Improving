---
name: landing-patterns-structural-page-architectures
description: Compose landings from proven patterns. Use when the user asks to architect tokens, bind palettes to categories, pair typefaces, specify components, or govern systems. Covers Section 5: Landing Patterns — Structural Page Architectures (Design Systems & Tokenization).
compatibility: token pipelines, Google Fonts API, component libraries, and changesets; assumes calibrated review conditions and non-destructive, reversible technique.
metadata:
...
# Landing Patterns — Structural Page Architectures

## Mission

Deliver the Section 5 capability of Design Systems & Tokenization: compose landings from proven patterns. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Compose landings from proven patterns.
- Justify structures by conversion logic.
- Hold responsiveness.

Context of this section: Reusable landing structures: hero, proof, feature, pricing, and footer patterns with conversion rationale and responsive rules.
...
Standard toolkit: token pipelines, Google Fonts API, component libraries, and changesets.

Available approaches for this section:

- **Tiered tokens** -- primitive to semantic to component
- **Category-bound color** -- palettes with contrast proof
- **Governed evolution** -- versioned, linted adoption
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 5: Landing Patterns — Structural Page Architectures via Tiered tokens." Do not blend approaches accidentally.
...
Work with token pipelines, Google Fonts API, component libraries, and changesets. Keep every consequential move on its own named, masked, reversible layer or stage. Record settings as logic with reasons, not as bare numbers, so the next piece in the set can adapt them honestly.

Typical working contexts for this skill:

1. dark mode derived systematically from one palette.
2. landing assembled purely from system patterns.
3. breaking token change deprecated without pain.

## Examples

**Example 1:** Input: dark mode derived systematically from one palette. Actions: declare the approach, rebuild the foundation, execute low-intensity precision passes, protect critical qualities, proof twice. Result: section-standard outcome with documented rationale.

**Example 2:** Input: landing assembled purely from system patterns. Actions: diagnose against capability targets, repair the foundation, re-apply intent, verify coherence. Result: rescued work meeting this section's bar.

**Example 3:** Input: breaking token change deprecated without pain. Actions: define the signature on the hero, propagate with adaptation, verify the set. Result: coherent series holding this section's standard throughout.
...
Avoid off-palette hex values in product code, unpaired typefaces chosen by whim, components forked instead of extended, breaking changes without deprecation, documentation contradicting shipped tokens.

Reference list:

- off-palette hex values in product code.
- unpaired typefaces chosen by whim.
- components forked instead of extended.
- breaking changes without deprecation.
- documentation contradicting shipped tokens.
...
6. Is every value traceable to a token?
7. Does the change follow deprecation policy?
8. Would a new team adopt this unaided?
...
Taste compiled into law. Tokenize everything, pair by rule, and version like software.
