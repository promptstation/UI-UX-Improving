---
name: master-plus-overrides-inheritance-without-duplication
description: Implement master-plus-overrides inheritance. Use when the user asks to design model-legible hierarchies, implement master-plus-overrides, budget context, or debug layout faults. Covers Section 2: Master Plus Overrides — Inheritance Without Duplication (Reasoning Engine Layouts).
compatibility: versioned schemas, master-plus-overrides datasets, validators, and session evals; assumes calibrated review conditions and non-destructive, reversible technique.
metadata:
  author: Promptstation
  version: 1.0.0
  category: llm-data-design
---

# Master Plus Overrides — Inheritance Without Duplication

## Mission

Deliver the Section 2 capability of Reasoning Engine Layouts: implement master-plus-overrides inheritance. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Implement master-plus-overrides inheritance.
- Define merge and conflict rules.
- Keep data DRY.

Context of this section: The Master plus Overrides pattern: base records, scoped override layers, deterministic merge rules, and conflict resolution the model can follow.
...
Standard toolkit: versioned schemas, master-plus-overrides datasets, validators, and session evals.

Available approaches for this section:

- **Hierarchical consumption** -- nested records over flat text
- **DRY inheritance** -- masters with scoped overrides
- **Validated sessions** -- schemas rejecting drift
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 2: Master Plus Overrides — Inheritance Without Duplication via Hierarchical consumption." Do not blend approaches accidentally.
...
Work with versioned schemas, master-plus-overrides datasets, validators, and session evals. Keep every consequential move on its own named, masked, reversible layer or stage. Record settings as logic with reasons, not as bare numbers, so the next piece in the set can adapt them honestly.

Typical working contexts for this skill:

1. coding session consuming ten thousand records hierarchically.
2. override layer changing one client without forks.
3. bad output traced to one layout section.

## Examples

**Example 1:** Input: coding session consuming ten thousand records hierarchically. Actions: declare the approach, rebuild the foundation, execute low-intensity precision passes, protect critical qualities, proof twice. Result: section-standard outcome with documented rationale.

**Example 2:** Input: override layer changing one client without forks. Actions: diagnose against capability targets, repair the foundation, re-apply intent, verify coherence. Result: rescued work meeting this section's bar.

**Example 3:** Input: bad output traced to one layout section. Actions: define the signature on the hero, propagate with adaptation, verify the set. Result: coherent series holding this section's standard throughout.
...
Avoid flat dumps exceeding context windows, duplicated data instead of overrides, prose constraints models ignore, unversioned schema changes breaking sessions, unvalidated outputs shipped as truth.

Reference list:

- flat dumps exceeding context windows.
- duplicated data instead of overrides.
- prose constraints models ignore.
- unversioned schema changes breaking sessions.
- unvalidated outputs shipped as truth.
...
6. Does the model consume this hierarchically?
7. Is every output schema-validated?
8. Can a fault be traced to one section?
...
Structure is instruction. Inherit, constrain, validate, and the model will reason where you point.
