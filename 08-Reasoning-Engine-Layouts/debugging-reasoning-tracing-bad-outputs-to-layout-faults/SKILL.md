---
name: debugging-reasoning-tracing-bad-outputs-to-layout-faults
description: Ablate layouts to isolate faults. Use when the user asks to design model-legible hierarchies, implement master-plus-overrides, budget context, or debug layout faults. Covers Section 7: Debugging Reasoning — Tracing Bad Outputs to Layout Faults (Reasoning Engine Layouts).
...
# Debugging Reasoning — Tracing Bad Outputs to Layout Faults

## Mission

Deliver the Section 7 capability of Reasoning Engine Layouts: ablate layouts to isolate faults. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Ablate layouts to isolate faults.
- Build minimal repros.
- Attribute faults correctly.

Context of this section: Ablation of layout sections, minimal reproduction cases, and distinguishing layout faults from model faults with controlled tests.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 7: Debugging Reasoning — Tracing Bad Outputs to Layout Faults via Hierarchical consumption." Do not blend approaches accidentally.
