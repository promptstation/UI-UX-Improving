---
name: motion-timing-and-media-vestibular-and-cognitive-safety
description: Honor reduced-motion preferences. Use when the user asks to audit against WCAG, fix contrast, repair keyboard flows, correct semantics, or certify AA. Covers Section 6: Motion, Timing and Media — Vestibular and Cognitive Safety (Digital Accessibility (a11y)).
compatibility: axe-style automation, screen readers, contrast analyzers, and reflow test rigs; assumes calibrated review conditions and non-destructive, reversible technique.
metadata:
...
# Motion, Timing and Media — Vestibular and Cognitive Safety

## Mission

Deliver the Section 6 capability of Digital Accessibility (a11y): honor reduced-motion preferences. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Honor reduced-motion preferences.
- Caption all meaningful media.
- Eliminate timing and flashing hazards.

Context of this section: Reduced-motion handling, timeout and autoplay rules, captions and transcripts, and flashing safeguards for vestibular and cognitive safety.
...
Standard toolkit: axe-style automation, screen readers, contrast analyzers, and reflow test rigs.

Available approaches for this section:

- **Evidence-led audit** -- template-scoped findings
- **Native-first repair** -- semantics before ARIA
- **Locked conformance** -- regression tests holding AA
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 6: Motion, Timing and Media — Vestibular and Cognitive Safety via Evidence-led audit." Do not blend approaches accidentally.
...
Work with axe-style automation, screen readers, contrast analyzers, and reflow test rigs. Keep every consequential move on its own named, masked, reversible layer or stage. Record settings as logic with reasons, not as bare numbers, so the next piece in the set can adapt them honestly.

Typical working contexts for this skill:

1. checkout flow made fully keyboard-operable.
2. dashboard passing 400 percent zoom.
3. conformance statement surviving scrutiny.

## Examples

**Example 1:** Input: checkout flow made fully keyboard-operable. Actions: declare the approach, rebuild the foundation, execute low-intensity precision passes, protect critical qualities, proof twice. Result: section-standard outcome with documented rationale.

**Example 2:** Input: dashboard passing 400 percent zoom. Actions: diagnose against capability targets, repair the foundation, re-apply intent, verify coherence. Result: rescued work meeting this section's bar.

**Example 3:** Input: conformance statement surviving scrutiny. Actions: define the signature on the hero, propagate with adaptation, verify the set. Result: coherent series holding this section's standard throughout.
...
Avoid color-only meaning, invisible or missing focus states, ARIA roles native elements already provide, keyboard traps in dialogs, untested claims of accessibility.

Reference list:

- color-only meaning.
- invisible or missing focus states.
- ARIA roles native elements already provide.
- keyboard traps in dialogs.
- untested claims of accessibility.
...
6. Is this operable keyboard-only?
7. Does meaning survive without color?
8. Would this pass with a screen reader?
...
Access is a feature, not a favor. Audit honestly, fix natively, and lock it with tests.
