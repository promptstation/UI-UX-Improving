---
name: semantics-and-aria-correct-roles
description: Choose native semantics by default. Use when the user asks to audit against WCAG, fix contrast, repair keyboard flows, correct semantics, or certify AA. Covers Section 4: Semantics and ARIA — Correct Roles (Digital Accessibility (a11y)).
...
# Semantics and ARIA — Correct Roles

## Mission

Deliver the Section 4 capability of Digital Accessibility (a11y): choose native semantics by default. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Choose native semantics by default.
- Apply ARIA only where native falls short.
- Announce dynamic changes properly.

Context of this section: Native elements first, landmarks, widget roles, live regions for dynamic content, and the first rule of ARIA in practice.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 4: Semantics and ARIA — Correct Roles via Evidence-led audit." Do not blend approaches accidentally.
