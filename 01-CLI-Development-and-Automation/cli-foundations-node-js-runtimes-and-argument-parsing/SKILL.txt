---
name: cli-foundations-node-js-runtimes-and-argument-parsing
description: Parse arguments and subcommands robustly. Use when the user asks to build parsers and subcommands, scaffold with init commands, edit system configs safely, or ship cross-platform CLIs. Covers Section 1: CLI Foundations — Node.js Runtimes and Argument Parsing (CLI Development & Automation).
compatibility: Node.js, TypeScript, and Bun with cross-platform CI matrices; assumes calibrated review conditions and non-destructive, reversible technique.
metadata:
...
# CLI Foundations — Node.js Runtimes and Argument Parsing

## Mission

Deliver the Section 1 capability of CLI Development & Automation: parse arguments and subcommands robustly. Work from evidence, protect what matters, and prove the result at full size before delivery.
...
Context of this section: Node.js CLI anatomy: argument parsing, flags and subcommands, stdin-stdout-stderr streams, exit codes, and signal handling, establishing how professional command-line tools communicate with users and scripts.
...
Standard toolkit: Node.js, TypeScript, and Bun with cross-platform CI matrices.

Available approaches for this section:

- **Interactive scaffolding** -- prompted init with dry-run previews
- **Safe mutation** -- diff preview, backup, and rollback on every write
- **Portable distribution** -- npm, bunx, and standalone binaries
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 1: CLI Foundations — Node.js Runtimes and Argument Parsing via Interactive scaffolding." Do not blend approaches accidentally.
...
Work with Node.js, TypeScript, and Bun with cross-platform CI matrices. Keep every consequential move on its own named, masked, reversible layer or stage. Record settings as logic with reasons, not as bare numbers, so the next piece in the set can adapt them honestly.

Typical working contexts for this skill:

1. uipro init scaffolding a project without touching existing files.
2. dotfile migration with previewed diffs and rollback.
3. global CLI released identically on three operating systems.

## Examples

**Example 1:** Input: uipro init scaffolding a project without touching existing files. Actions: declare the approach, rebuild the foundation, execute low-intensity precision passes, protect critical qualities, proof twice. Result: section-standard outcome with documented rationale.

**Example 2:** Input: dotfile migration with previewed diffs and rollback. Actions: diagnose against capability targets, repair the foundation, re-apply intent, verify coherence. Result: rescued work meeting this section's bar.

**Example 3:** Input: global CLI released identically on three operating systems. Actions: define the signature on the hero, propagate with adaptation, verify the set. Result: coherent series holding this section's standard throughout.
...
Avoid mutating user systems without dry-run previews, untested platform-specific path handling, unvalidated config writes with no rollback, opaque errors without actionable next steps, distribution without checksums or version channels.

Reference list:

- mutating user systems without dry-run previews.
- untested platform-specific path handling.
- unvalidated config writes with no rollback.
- opaque errors without actionable next steps.
- distribution without checksums or version channels.
...
6. Was every mutation previewed and reversible?
7. Does it behave identically on all three OS families?
8. Can a new user recover from any error unaided?
...
Respect the machine it runs on. Preview everything, write reversibly, and behave identically everywhere.
