---
name: mobile-toolkits-swiftui-and-jetpack-compose
description: Build equivalent features in SwiftUI and Compose. Use when the user asks to map stack architectures, build per-stack features, enforce cross-stack parity, or share logic natively. Covers Section 5: Mobile Toolkits — SwiftUI and Jetpack Compose (Multi-Platform Front-End Stacks).
...
# Mobile Toolkits — SwiftUI and Jetpack Compose

## Mission

Deliver the Section 5 capability of Multi-Platform Front-End Stacks: build equivalent features in SwiftUI and Compose. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Build equivalent features in SwiftUI and Compose.
- Handle platform services.
- Prepare store submissions.

Context of this section: Declarative mobile UI on both platforms: state, navigation, lists, and platform services, plus store-readiness requirements.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 5: Mobile Toolkits — SwiftUI and Jetpack Compose via Spec-driven builds." Do not blend approaches accidentally.
