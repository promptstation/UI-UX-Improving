---
name: desktop-frameworks-javafx-avalonia-and-winui-3
description: Build native-feel desktop apps in each framework. Use when the user asks to map stack architectures, build per-stack features, enforce cross-stack parity, or share logic natively. Covers Section 4: Desktop Frameworks — JavaFX, Avalonia and WinUI 3 (Multi-Platform Front-End Stacks).
compatibility: Next.js, Nuxt, Avalonia, WinUI 3, SwiftUI, Jetpack Compose, and screenshot-diff testing; assumes calibrated review conditions and non-destructive, reversible technique.
metadata:
...
# Desktop Frameworks — JavaFX, Avalonia and WinUI 3

## Mission

Deliver the Section 4 capability of Multi-Platform Front-End Stacks: build native-feel desktop apps in each framework. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Build native-feel desktop apps in each framework.
- Package and update per OS.
- Respect desktop UX conventions.

Context of this section: XAML and FXML UI models, MVVM patterns, native packaging and auto-update per OS, and desktop-specific UX such as menus and windowing.
...
Standard toolkit: Next.js, Nuxt, Avalonia, WinUI 3, SwiftUI, Jetpack Compose, and screenshot-diff testing.

Available approaches for this section:

- **Spec-driven builds** -- one brief, many idiomatic shells
- **Token-shared visuals** -- identical design, native code
- **Parity-tested** -- screenshot diffs gating changes
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 4: Desktop Frameworks — JavaFX, Avalonia and WinUI 3 via Spec-driven builds." Do not blend approaches accidentally.
...
Work with Next.js, Nuxt, Avalonia, WinUI 3, SwiftUI, Jetpack Compose, and screenshot-diff testing. Keep every consequential move on its own named, masked, reversible layer or stage. Record settings as logic with reasons, not as bare numbers, so the next piece in the set can adapt them honestly.

Typical working contexts for this skill:

1. same landing shipped on web and desktop.
2. mobile pair passing behavioral parity tests.
3. shared client regenerated across stacks.

## Examples

**Example 1:** Input: same landing shipped on web and desktop. Actions: declare the approach, rebuild the foundation, execute low-intensity precision passes, protect critical qualities, proof twice. Result: section-standard outcome with documented rationale.

**Example 2:** Input: mobile pair passing behavioral parity tests. Actions: diagnose against capability targets, repair the foundation, re-apply intent, verify coherence. Result: rescued work meeting this section's bar.

**Example 3:** Input: shared client regenerated across stacks. Actions: define the signature on the hero, propagate with adaptation, verify the set. Result: coherent series holding this section's standard throughout.
...
Avoid one stack drifting from the shared spec, non-idiomatic ports fighting the platform, visual parity asserted without tests, shared logic copy-pasted per stack, platform conventions ignored.

Reference list:

- one stack drifting from the shared spec.
- non-idiomatic ports fighting the platform.
- visual parity asserted without tests.
- shared logic copy-pasted per stack.
...
6. Is each shell idiomatic to its platform?
7. Do parity tests gate this change?
8. Is shared logic generated, not copied?
...
One product, many homes. Share the logic, honor each platform, prove parity with tests.
