---
name: parity-engineering-same-design-across-stacks
description: Drive multiple stacks from one spec. Use when the user asks to map stack architectures, build per-stack features, enforce cross-stack parity, or share logic natively. Covers Section 6: Parity Engineering — Same Design Across Stacks (Multi-Platform Front-End Stacks).
...
# Parity Engineering — Same Design Across Stacks

## Mission

Deliver the Section 6 capability of Multi-Platform Front-End Stacks: drive multiple stacks from one spec. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Drive multiple stacks from one spec.
- Enforce parity with visual tests.
- Propagate changes cleanly.

Context of this section: Spec-driven multi-stack builds, shared design tokens, screenshot-diff testing, and change-propagation workflows keeping implementations aligned.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 6: Parity Engineering — Same Design Across Stacks via Spec-driven builds." Do not blend approaches accidentally.
