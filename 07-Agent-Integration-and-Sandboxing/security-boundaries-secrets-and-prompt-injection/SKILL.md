---
name: security-boundaries-secrets-and-prompt-injection
description: Redact secrets from agent context. Use when the user asks to map agent context, author rules, package plugins, sandbox execution, or evaluate compliance. Covers Section 7: Security Boundaries — Secrets and Prompt Injection (Agent Integration & Sandboxing).
...
# Security Boundaries — Secrets and Prompt Injection

## Mission

Deliver the Section 7 capability of Agent Integration & Sandboxing: redact secrets from agent context. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Redact secrets from agent context.
- Defend against indirect injection.
- Respond to misuse incidents.

Context of this section: Secret redaction from context, untrusted-content handling, indirect prompt-injection defenses, and incident response for agent misuse.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 7: Security Boundaries — Secrets and Prompt Injection via Standardized rules." Do not blend approaches accidentally.
