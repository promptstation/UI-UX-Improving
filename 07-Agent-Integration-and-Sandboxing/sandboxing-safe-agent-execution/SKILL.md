---
name: sandboxing-safe-agent-execution
description: Scope agent permissions tightly. Use when the user asks to map agent context, author rules, package plugins, sandbox execution, or evaluate compliance. Covers Section 4: Sandboxing — Safe Agent Execution (Agent Integration & Sandboxing).
...
# Sandboxing — Safe Agent Execution

## Mission

Deliver the Section 4 capability of Agent Integration & Sandboxing: scope agent permissions tightly. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Scope agent permissions tightly.
- Gate destructive actions.
- Audit sessions from logs.

Context of this section: Permission models, filesystem and network scoping, approval gates for destructive actions, and audit logs for agent sessions.
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 4: Sandboxing — Safe Agent Execution via Standardized rules." Do not blend approaches accidentally.
