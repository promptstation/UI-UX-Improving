---
name: agent-landscape-how-coding-agents-consume-context
description: Map context pipelines per agent. Use when the user asks to map agent context, author rules, package plugins, sandbox execution, or evaluate compliance. Covers Section 1: Agent Landscape — How Coding Agents Consume Context (Agent Integration & Sandboxing).
compatibility: Claude Code, Cursor, Windsurf, Copilot configs, sandbox policies, and compliance suites; assumes calibrated review conditions and non-destructive, reversible technique.
metadata:
...
# Agent Landscape — How Coding Agents Consume Context

## Mission

Deliver the Section 1 capability of Agent Integration & Sandboxing: map context pipelines per agent. Work from evidence, protect what matters, and prove the result at full size before delivery.

Capability targets for this skill:

- Map context pipelines per agent.
- Predict what each model sees.
- Choose integration points deliberately.

Context of this section: How Claude Code, Cursor, Windsurf, and Copilot ingest system prompts, rules files, and plugins, and what each surfaces to the model per session.
...
Standard toolkit: Claude Code, Cursor, Windsurf, Copilot configs, sandbox policies, and compliance suites.

Available approaches for this section:

- **Standardized rules** -- one standard ported four ways
- **Least-privilege sandbox** -- approvals for destruction
- **Evaluated behavior** -- suites proving compliance
...
Choose exactly one approach per piece from the list above. Record it in one line with the section number, for example: "Section 1: Agent Landscape — How Coding Agents Consume Context via Standardized rules." Do not blend approaches accidentally.
...
Work with Claude Code, Cursor, Windsurf, Copilot configs, sandbox policies, and compliance suites. Keep every consequential move on its own named, masked, reversible layer or stage. Record settings as logic with reasons, not as bare numbers, so the next piece in the set can adapt them honestly.

Typical working contexts for this skill:

1. repo rules adopted identically across four agents.
2. destructive command stopped at approval gate.
3. model update caught by regression prompts.

## Examples

**Example 1:** Input: repo rules adopted identically across four agents. Actions: declare the approach, rebuild the foundation, execute low-intensity precision passes, protect critical qualities, proof twice. Result: section-standard outcome with documented rationale.

**Example 2:** Input: destructive command stopped at approval gate. Actions: diagnose against capability targets, repair the foundation, re-apply intent, verify coherence. Result: rescued work meeting this section's bar.

**Example 3:** Input: model update caught by regression prompts. Actions: define the signature on the hero, propagate with adaptation, verify the set. Result: coherent series holding this section's standard throughout.
...
Avoid unscoped rules contradicting each other, agents with destructive unapproved access, secrets leaked into model context, untrusted content executed unexamined, unevaluated claims of agent behavior.

Reference list:

- unscoped rules contradicting each other.
- agents with destructive unapproved access.
- secrets leaked into model context.
- untrusted content executed unexamined.
- unevaluated claims of agent behavior.
...
6. Is every destructive action gated?
7. Are secrets absent from context?
8. Do evals prove the claimed behavior?
...
Agents are powerful interns. Scope them tightly, evaluate constantly, and never trust blindly.
